...
const TrendingCards = () => {
  ...
  const fetchProducts = useCallback(async () => {
    try {
      const res = await axios.get(`${API_BASE_URL}/api/user/card/getAllCards?isTrending=true`);
      const trending = res.data.data?.filter(card => card.isTrending === true) || [];
      setTrendingCards(trending);
    } catch (error) {
      console.error("Issue while fetching trending products", error);
      setTrendingCards([]);
    }
  }, []);
  ...
  return (
    <div className="px-4 md:px-16 py-10 bg-gradient-to-b from-white to-gray-50 font-serif">
      ...
      <h2 className="text-3xl md:text-4xl font-bold mb-8 text-center text-gray-800 relative">
        <span className="relative z-10">🚀 Trending Cards</span>
