...
function Header() {
  const { showLogin, setShowLogin, user, fetchUser } = useContext(AuthContext);
  ...
  // ✅ Fetch user only once on mount (prevents blinking)
  useEffect(() => {
    fetchUser();
  }, []);

  // ✅ Search logic runs separately without re-fetching user
  useEffect(() => {
    if (!searchQuery.trim()) {
      setSuggestions([]);
      return;
